...
Picture
drw_picture_create_resized(Drw *drw, char *src, unsigned int srcw,
	...
	if(srcw <= (dstw << 1u) && srch <= (dsth << 1u)) {
		...
		pic = XRenderCreatePicture(drw->dpy, pm,
					   XRenderFindStandardFormat(drw->dpy,
								     PictStandardARGB32),
					   0, NULL);
		XFreePixmap(drw->dpy, pm);

		XRenderSetPictureFilter(drw->dpy, pic, FilterBilinear, NULL,
					0);
		XTransform xf;
		...
		Imlib_Image origin =
			imlib_create_image_using_data(srcw, srch,
						      (DATA32 *) src);
		if(!origin)
			return None;
		...
		Imlib_Image scaled =
			imlib_create_cropped_scaled_image(0, 0, srcw, srch,
							  dstw, dsth);
...
drw_rect(Drw *drw, int x, int y, unsigned int w, unsigned int h, int filled,
	...
	XSetForeground(drw->dpy, drw->gc,
		       invert ? drw->scheme[ColBg].pixel : drw->scheme[ColFg].
		       pixel);
	if(filled)
...
drw_pic(Drw *drw, int x, int y, unsigned int w, unsigned int h, Picture pic) {
    if (!drw || !pic || !drw->picture)  /* guard both src and dst */
        return;
...
void
drw_polygon(Drw *drw, int x, int y, int ow, int oh, int sw, int sh,
